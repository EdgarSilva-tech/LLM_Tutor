...
def test_format_evaluator_prompt():
    """
    Tests if the evaluator prompt is formatted correctly with the question and answer.
    """
...
def test_get_llm(MockChatOpenAI):
    ...
    # Assert that the ChatOpenAI class was called once with the expected arguments
...
def test_eval_answer_exception_handling(mock_get_llm):
    """
    Tests that the try-except block in eval_answer correctly catches and returns exceptions.
    """
    # --- Arrange ---
