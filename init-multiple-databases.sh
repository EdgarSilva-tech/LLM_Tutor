...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    SELECT 'CREATE DATABASE "Quizz"' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'Quizz')\gexec
    SELECT 'CREATE DATABASE "LearningAssessment"' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'LearningAssessment')\gexec
    ...
    GRANT ALL PRIVILEGES ON DATABASE "Quizz" TO $POSTGRES_USER;
    GRANT ALL PRIVILEGES ON DATABASE "LearningAssessment" TO $POSTGRES_USER;
EOSQL
